...
class WriteData(object):
    ...
    def insert_data(self, callnumber, tag, tagnum):
        cursor = self.conn.cursor()
        try:
            sql = "insert into spidertable(callnumber,baidutype,baiducount) values('%s','%s','%s')" %(callnumber,tag,tagnum)
            cursor.execute(sql)
    ...
    def update_data(self,callnumber,tag,tagnum):
        cursor = self.conn.cursor()
        try:
            sql = "update spidertable set baidutype='%s',baiducount='%s' where callnumber='%s' "%(tag,tagnum,callnumber)
            cursor.execute(sql)
        finally:
            cursor.close()
