...
class WriteData(object):
    ...
    def insert_data(self, callnumber, tag, tagnum):
        cursor = self.conn.cursor()
        try:
            sql = "insert into spidertable(callnumber,360type,360count) values('%s','%s','%s')" %(callnumber,tag,tagnum)
            cursor.execute(sql)
    ...
    def update_data(self,callnumber,tag,tagnum):
        cursor = self.conn.cursor()
        try:
            sql = "update spidertable set 360type='%s',360count='%s' where callnumber='%s' "%(tag,tagnum,callnumber)
            cursor.execute(sql)
        finally:
            cursor.close()
